...
public:
	...
	static bool read_num_class_data(const std::string& filename, int var_count,cv::Mat* _data, cv::Mat* _responses)
	{
		using namespace cv;
		using namespace std;
		const int M = 1024;
	...
	static cv::Ptr<cv::ml::TrainData> prepare_train_data(const cv::Mat& data, const cv::Mat& responses, int ntrain_samples)
	{
		using namespace cv;
		using namespace cv::ml;
		...
		var_type.setTo(Scalar::all(cv::ml::VAR_ORDERED));
	...
	static inline cv::TermCriteria TC(int iters, double eps)
	{
		using namespace cv;
		using namespace cv::ml;
	...
	static void test_and_save_classifier(const cv::Ptr<cv::ml::StatModel>& model,
		const cv::Mat& data, const cv::Mat& responses,
		int ntrain_samples, int rdelta,
		const std::string& filename_to_save)
	{
		using namespace cv;
		using namespace cv::ml;
	...
	static bool build_rtrees_classifier(const std::string& data_filename,
			const std::string& filename_to_save,
			const std::string& filename_to_load)
	{
		using namespace cv;
		using namespace cv::ml;
		using namespace std;
	...
	static bool
		build_boost_classifier(const std::string& data_filename,
			const std::string& filename_to_save,
			const std::string& filename_to_load)
	{
		using namespace cv;
	...
	static bool build_mlp_classifier(const std::string& data_filename, const std::string& filename_to_save, const std::string& filename_to_load)
	{
		using namespace cv;
		using namespace cv::ml;
		using namespace std;
	...
	static bool build_knearest_classifier(const std::string& data_filename, int K)
	{
		using namespace cv;
		using namespace cv::ml;
		using namespace std;
	...
	static bool build_nbayes_classifier(const std::string& data_filename)
	{
		using namespace cv;
		using namespace cv::ml;
		using namespace std;
	...
	static bool build_svm_classifier(const std::string& data_filename, const std::string& filename_to_save, const std::string& filename_to_load)
	{
		using namespace cv;
		using namespace cv::ml;
		using namespace std;
