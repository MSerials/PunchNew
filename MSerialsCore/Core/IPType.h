...
typedef UINT1 MBOOL; /* TRUE (1) or FALSE (0) */
typedef UINT1 MBYTE; /* unsigned Byte */

#if !defined(HC_LARGE_IMAGES)
typedef INT4 MIMGDIM;   /* Image dimension, e.g., width and height */
typedef INT2 MIMGCOOR;  /* Image coordinates, e.g, row and column */
typedef INT4 MLINCOOR;  /* Linearized image coordinates */
typedef INT4 MIMGCNT;   /* Number of pixels, e.g., area or buffer size */
typedef INT4 MITEMCNT;  /* Number of elements, e.g., contours or runlengths */
typedef float MSUBCOOR; /* Sub-pixel precise coordinates */
typedef float MSUBATTR; /* Sub-pixel precise attributes of contours */
#else
typedef INT4 MIMGDIM;
typedef INT4 MIMGCOOR;
typedef INT4_8 MLINCOOR;
typedef INT4_8 MIMGCNT;
typedef INT4_8 MITEMCNT;
typedef double MSUBCOOR;
typedef double MSUBATTR;
#endif
...
typedef union {
    MBYTE *b;         /* 0..255 (BYTE_IMAGE)                          */
    MBYTE *z;         /* 0..255 mod 256 (CYCLIC_IMAGE)                */
    MBYTE *d;         /* orientation 0..180 (DIR_IMAGE)               */
...
typedef struct
{
    ...
    MBOOL free;               /* free image data when deleting image          */
...
typedef struct
{
    MBOOL is_compl;      /* region is complement                         */
...
typedef struct
{
    ...
    MBOOL *no_row;     /* is there this row available?                 */
} Hrlaccess;
...
typedef struct
{
    ...
    MBOOL is_equidist; /* Are the x-values equidistant?                      */
} Hfunction_1d;
